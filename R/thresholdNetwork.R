...
#' @return a dataframe
#' @export
rankBasedThresholdNetwork = function(MAT, mix=FALSE, d = 3,returnGraph=FALSE) {
    ...
    if(returnGraph==FALSE){
      return(as.data.frame(cbind(edgeNodeRatioAs,norGiantSizeAs,meanClusterSizeAs,edgeNodeRatioDe,norGiantSizeDe,meanClusterSizeDe)))
    }else{
      return(list(GRAPH_As,GRAPH_De,as.data.frame(cbind(edgeNodeRatioAs,norGiantSizeAs,meanClusterSizeAs,edgeNodeRatioDe,norGiantSizeDe,meanClusterSizeDe))))
    }
